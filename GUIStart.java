...
public class GUIStart extends JFrame {
    ...
    private float[][] dataNormalized = null;
    private boolean readData = false;
    private boolean normalization = false;

    private float[][] dataSelected;
    private float[][] dataNormSelected;
    private String[] attributeSelectedName;
    private boolean selectData = false;
    private boolean[] selectDimension;
    private int k = 0;
    private int scaleStart = 1;
    ...
    private Object[][] transferTableData(float[][] tableData, int[] lineId) {
    ...
    private JTable buildTable(float[][] tableData, String[] attributes, int rowStart) {
        int n = tableData.length;
        int d = tableData[0].length;
    ...
    public GUIStart(int method) {
        ...
        read.addActionListener(o -> {
            ...
            if (files == null || files.length != 1) {
                success = false;
                warning("Wrong File! Please Select The Right Formatted File or Choose One File!", "File Read Warning");
            } else {
                try {
                    this.data = FileHandler.readFile(files[0]);
                } catch (Exception e) {
                    success = false;
            ...
            if (!this.readData) {
                ...
                confirm.addActionListener(o2 -> {
                    ...
                    if (scaleStartTmp < 0 || scaleEndTmp < 0 || kTmp < 0) {
                        warning("Please Check Your Input!", "Illegal Number");
...
//                JPanel jPanel = new JPanel(new FlowLayout(FlowLayout.LEADING, 20, 20));
        ...
        run.addActionListener(o -> {
            if (!this.readData) {
                ...
                iqsRec.initData(this.dataNormSelected);
                int[] skyline = iqsRec.getSkyline(k, method);

                float[][] values = new float[skyline.length][this.dataSelected[0].length];
    ...
    public static void main(String[] args) {
        //  0: choose top candidateNum after sorting in descending, when dataset is large and m is small, it can be close to true point
